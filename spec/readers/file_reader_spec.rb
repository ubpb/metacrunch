describe Metacrunch::Readers::FileReader do
  ...
  it "can read a single file" do
    reader = Metacrunch::Readers::FileReader.new(regular_file)
    expect(reader.each.count).to be(1)
  end

  it "can read multiple files" do
    reader = Metacrunch::Readers::FileReader.new([regular_file, regular_file])
  ...
  it "can read regular files" do
    ...
    reader.each do |entry|
      expect(entry).to be_instance_of(Metacrunch::Readers::FileReader::Entry)
  ...
  it "can read compressed regular files" do
    reader = Metacrunch::Readers::FileReader.new(compressed_regular_file)
    reader.each do |entry|
      expect(entry).to be_instance_of(Metacrunch::Readers::FileReader::Entry)
  ...
  it "can read TAR archives" do
    reader = Metacrunch::Readers::FileReader.new(archive_file)
    reader.each do |entry|
  ...
  it "can read compressed TAR archives" do
    reader = Metacrunch::Readers::FileReader.new(compressed_archive_file)
    reader.each do |entry|
      expect(entry).not_to be_nil
      expect(entry).to be_instance_of(Metacrunch::Readers::FileReader::Entry)
